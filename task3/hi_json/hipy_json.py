...
class Json_Parser:
    ...
    def json_delete(self, data: dict) -> list:
        lst = []
        for k, v in data.items():
            if isinstance(v, dict):
                lst.append(f"{k}: {self.json_delete(v)}")
            else:
                lst.append(f"{k}: {v}")

        res =  ', '.join(lst)
        result = re.sub(r'project:|shot:|EP0002', '',res)
        return result
...
if __name__ == '__main__':
    ...
    f = Json_Parser('/home/jun/clone/team_A/task3/hi_json/json_file.json')
    fi = f.open_file()
    ...
    if a == '1':
        pprint.pp(f.json_modify(fi, modify_key_data, c_data))
    elif a == '2':
        pprint.pp(f.json_delete(fi))
    elif a == '3':
        pprint.pp(f.json_insert(fi, insert_key_data),sort_dicts=False)
    elif a == '4':
        pprint.pp((f.json_get(fi,'datetime')))
